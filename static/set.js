...
var food_set_ops = {
    ...
    eventBind: function () {
        ...
        $(".wrap_food_set .save").click(function () {
            ...
            var mileage_target = $(".wrap_food_set input[name=mileage]");
            var mileage = mileage_target.val();
            ...
            if (parseInt(cat_id) < 1) {
                common_ops.tip("请选择品牌!", cat_id_target);
                return;
            }

            if (name.length < 1) {
                common_ops.alert("请输入标题名称!");
                return;
            }

            if (parseFloat(price) <= 0) {
                common_ops.tip("请输入二手价格!", price_target);
                return;
            }

            // if (mileage.length <= 0) {
            //     common_ops.tip("请输入车辆行驶里程!", mileage_target);
            //     return;
            // }
            ...
            if (summary.length < 10) {
                common_ops.alert("请输入详情描述，不少于10个字符!", price_target);
                return;
            }

            if (parseInt(stock) < 1) {
                common_ops.tip("请输入车辆数量!", stock_target);
                return;
            }
